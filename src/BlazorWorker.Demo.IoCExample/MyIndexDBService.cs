    ...
    public class MyIndexDBService
    {
        ...
        public async Task<string> GetPersonName(long id)
        {
            try
            {
                ...
                //output += $"{rn}{LogDate()} retrieving test person...";
                //Console.WriteLine($"{nameof(GetPersonName)}({id}): GetRecords from store '{storeName}'");
            ...
            {
                Console.WriteLine($"{nameof(GetPersonName)} :{e})");
                throw;
            }
        }
